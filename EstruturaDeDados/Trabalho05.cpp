...
namespace trabalho05
{
	string listaAtualTexto = "Débitos";
	char opcao;
	string mensagem = "";
	ListaEncadeadaSimples<Lancamento> listaCredito;
	ListaEncadeadaSimples<Lancamento> listaDebito;
	ListaEncadeadaSimples<Lancamento> *listaAtual = &listaDebito;
	...
	void listarTran() {
		if ((*listaAtual).EstruturaVazia())
			...
			for (int i = 0; i <= listaAtual->ObterIndiceTopo(); i++) {
				cout << i << " " << *(listaAtual->ObterDadoPelaPosicao(i));
			}
		}
	}
	...
	void mostraSaldo() {
		...
		for (int i = 0; i <= listaCredito.ObterIndiceTopo(); i++)
			tCredito += listaCredito.ObterDadoPelaPosicao(i)->Valor();
		for (int i = 0; i <= listaDebito.ObterIndiceTopo(); i++)
			tDebito += listaDebito.ObterDadoPelaPosicao(i)->Valor();
